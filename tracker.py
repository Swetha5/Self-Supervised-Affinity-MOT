...
class SSTTracker:
    ...
    def load_model(self):
        ...
        print('loading model :' + args.resume)
        if self.cuda:
            cudnn.benchmark = True
            self.sst.load_state_dict(torch.load(args.resume))
            self.sst = self.sst.cuda()
        else:
            self.sst.load_state_dict(torch.load(args.resume, map_location='cpu'))
        self.sst.eval()
