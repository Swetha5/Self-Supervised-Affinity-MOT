...
class PatchwiseDataset(Dataset):
    ...
    def _generate_img_lidar(self, idx, use_gt):
        ...
        first_flag = 0
        #for (frame, gt_frame) in zip(frames, gt_frames):
        #data_augmentation_lidar=Falase
        data_augmentation_lidar=random.choice([True,False])
        ...
        for fr in range(2):
            ...
            (frame, gt_frame)=(frames[jr], gt_frames[jr])
            path = f"{self.root_dir}/image_02/{frame['image_path']}"
            img = Image.open(path)
            ...
            for i in range(frame['detection']['bbox'].shape[0]):
                ...
                det_imgs.append(
                    self.transform(img.crop((x1, y1, x2, y2)).resize((224, 224), Image.BILINEAR)).unsqueeze(0))
                        #Augmentaion
            if data_augmentation_lidar:
                random_translate = random.choice([10,20,40]) 
                random_scale=random.choice([0.95,1.05,0.90,1.10])
                #a=np.ones((12,3))
                point_cloud['points']=point_cloud['points']*random_scale
                point_cloud['points']=point_cloud['points']+[random_translate,random_translate,random_translate]

                if flip:
                    point_cloud['points']=point_cloud['points']*[1,-1,1]
    ...
    def _generate_meta(self):
        print('form designated patch file')
        metas = []
