...
class TrackingModule(object):
    ...
    def predict(self, det_imgs, det_info, dets, det_split):
        ...
        #gt_det, gt_link, gt_new, gt_end=self.generate_gt(det_score[0], det_cls, det_id, det_split)

        #det_score, link_score, new_score, end_score, _ = self.model(
        #    det_imgs, det_info, a, b, det_split)
        ...
        normalized=(2*(out-torch.min(out))/(torch.max(out)-torch.min(out)))-1
        link_score=normalized[0][0][:det_split[0].item(),:det_split[1].item()]
        link_score=[torch.cat((link_score,link_score,link_score)).view(3,link_score.shape[0],link_score.shape[1])]
        #print('link_score',link_score[0].shape)

        
        det_score=torch.ones(det_split[0].item()+det_split[1].item(),device='cuda')
        det_score=torch.cat((det_score,det_score,det_score),0).view(3,det_score.shape[0])
        #print('det_score ', det_score.shape)
        '''
        #new
        labels=(2*(out-torch.min(out))/(torch.max(out)-torch.min(out)))-1
        #labels=(out-torch.min(out))/(torch.max(out)-torch.min(out))
        #creating gt_end
        column=labels[0][0][0:81,80]
        #print(column)
        gt_end=column[:det_split[0].item()]
        total=det_split[0].item()+det_split[1].item()
        m = nn.ConstantPad1d((0, total-det_split[0].item()), 1) 
        end_score=m(gt_end)
        end_score = torch.cat((end_score, end_score, end_score), 0).view(3,end_score.shape[0])


        #creating gt_new
        row=labels[0][0][80,0:81]
        #print(row)
        gt_new=row[:det_split[1].item()]
        m = nn.ConstantPad1d((0, total-det_split[1].item()), 1)
        new_score=m(gt_new)
        new_score = torch.cat((new_score, new_score, new_score), 0).view(3,new_score.shape[0])


        '''
        #first
        '''
        end_score=normalized[0][0][:,80][:det_split[0].item()+det_split[1].item()]
        end_score = torch.cat((end_score, end_score, end_score), 0).view(3,end_score.shape[0])
        #print('end_score ',end_score.shape)


        new_score=normalized[0][0][80,:][:det_split[1].item()+det_split[0].item()]
        new_score = torch.cat((new_score, new_score, new_score), 0).view(3,new_score.shape[0])
        #print('new_score ',new_score.shape)
        '''

        #second

        end_score=-1*torch.ones(det_split[0].item()+det_split[1].item(),device='cuda')
        end_score = torch.cat((end_score, end_score, end_score), 0).view(3,end_score.shape[0])
        #print('end_score ',end_score.shape)

        new_score=-1*torch.ones(det_split[0].item()+det_split[1].item(),device='cuda')
        new_score = torch.cat((new_score, new_score, new_score), 0).view(3,new_score.shape[0])
        #print('new_score ',new_score.shape)
