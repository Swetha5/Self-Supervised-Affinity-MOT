...
parser = argparse.ArgumentParser(description='PyTorch PC-DAN Self-Supervised Testing')
...
def main():
    ...
    print('dataset set built as test_dataset_Cars.pt')

    # test_dataset=torch.load('dataset_pt_files/test_dataset_2D_modified2.pt')
...
def validate_seq(val_loader,
    ...
    with torch.no_grad():
        for i, (input, det_info, dets, det_split) in enumerate(val_loader):
            ...
            # compute output
            #print('det_split', det_split)
            #print('fram_id ', det_info['info_id'])
            #print('dim ',det_info['dim'])
