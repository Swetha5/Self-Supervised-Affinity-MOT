    ...
    def update(self, sst, frame_index, features, boxes):
        ...
        if frame_index not in self.all_frame_index:
            ...
            for pre_index in self.all_frame_index[:-1]:
                ...
                pre_similarity = sst.forward_stacker_features(Variable(self.all_features[pre_index]),
...
class Track:
    ...
    def __init__(self):
        self.s = np.zeros((TrackerConfig.max_track_node, TrackerConfig.max_track_node),
                          dtype=np.float)  # similarity score
...
class TrackSet:
    ...
    def update(self, image, detection, show_image):
        ...
        self.tracks = [t for t in self.tracks if
                       t.age < TrackerConfig.max_track_age and t.get_bad_probability() < TrackerConfig.max_bad_node]
