...
def evaluate(groundtruths, tests, loglevel='info', fmt='mot15-2D', depth=True,
             solver='ortools'):
    ...
    if not depth:
        ...
        tsfiles = [f.replace('_3d','') for f in glob.glob(os.path.join(tests+'/JRDB_formate', '*.txt')) if(not os.path.basename(f).startswith('eval') and not 'result' in os.path.basename(f))]
    ...
    indoor_outdoor_split = {'INDOOR':['bytes-cafe-2019-02-07_0'],'OUTDOOR':[]}
    ...
    filename=tests+'/'+'summary.txt'
    save_summary=summary.to_string()
    #save_summary.to_csv(filename, header=None, index=None, mode='a')
    with open(filename, 'w') as f:
        f.write(save_summary)
    print('evaluation results saved in directory ', filename)
    return summary

if __name__ == '__main__':
    parser = argparse.ArgumentParser(formatter_class=argparse.RawTextHelpFormatter)
    parser.add_argument('groundtruths',default='/home/aakash/Downloads/JRDB-SAMOT/train_dataset/ground_truth', help='Directory containing ground truth files.')
    parser.add_argument('tests',default='/home/aakash/Downloads/JRDB-SAMOT/results/output1/last/val',help='Directory containing result files.')
    parser.add_argument('--loglevel', type=str, help='Log level', default='info')
    parser.add_argument('--fmt', type=str, help='Data format', default='mot15-2D')
    parser.add_argument('-d', '--depth',default=True, action='store_true', help='Whether evaluating in 3D')
    parser.add_argument('--solver', type=str, help='LAP solver to use', default='ortools')
