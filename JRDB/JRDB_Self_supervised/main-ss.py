...
def main():
	...
	#save_folder='ablation/without_random_drop/v_trans_rot_major_cutout_correction'
	#log_address='ablation/without_random_drop/v_trans_rot_major_cutout_correction/log'
	...
	for i, (input, det_info, det_id, det_cls,det_split) in enumerate(train_loader):
		#print('points',det_info['points'].shape)
		#print('det_id',det_id)
		#print('det_split', det_split[0])
		#print('det_split', det_split[1])
		...
		if current_step%(train_data_length*1)==0:
			...
			torch.save(net.state_dict(),
					   os.path.join(
						   save_weights,
						   'ss_JRDB_' +'epoch_'+ repr(actual_epoch)+ '_Loss_ %.4f' % (loss.item())+'.pth'))
		if current_step%(int(train_data_length*0.25))==0:
		#if (current_step)%100==0:
