...
def parse_arguments():
    ...
    parser.add_argument('--test', default='/home/c3-0/datasets/JRDB-new-version/test_dataset_without_labels', help='Directory of testing dataset')
    parser.add_argument('--train', default='/home/c3-0/datasets/JRDB-new-version/train_dataset_with_activity', help='Directory of training dataset')
    parser.add_argument('-o', '--outdir', default='/home/c3-0/datasets/JRDB-new-version/processed_data', help='Directory of MOT output')
...
def copy_images_and_pointclouds(opt):
    ...
    camera_names = ['image_0', 'image_2', 'image_4', 'image_6', 'image_8', 'image_stitched']
    ...
    for seq_folder, sequence_paths, indir in zip(sequence_folders_mot, sequence_paths_all, indirs):
        for i, seq_path in enumerate(sequence_paths):
            ...
            path_to_pcs_lower = os.path.join(indir, 'pointclouds/lower_velodyne', seq_name)
            lower_pc_paths = sorted(glob(os.path.join(path_to_pcs_lower, '*.pcd')))

            path_to_pcs_upper = os.path.join(indir, 'pointclouds/upper_velodyne', seq_name)
            ...
            for pc_src_lower, pc_src_upper in zip(lower_pc_paths, upper_pc_paths):
                ...
                pc_dst = os.path.splitext(pc_dst)[0] +'.pt'
                ...
                upper_pc -= np.expand_dims(np.array(global_config_dict['calibrated']['lidar_upper_to_rgb']['translation']), 0)
                upper_theta = float(global_config_dict['calibrated']['lidar_upper_to_rgb']['rotation'][-1])
                upper_rotation_matrix = np.array([[np.cos(upper_theta), -np.sin(upper_theta)], [np.sin(upper_theta), np.cos(upper_theta)]])
                upper_pc[:, :2] = np.squeeze(np.matmul(upper_rotation_matrix, np.expand_dims(upper_pc[:, :2], 2)))

                lower_pc -= np.expand_dims(np.array(global_config_dict['calibrated']['lidar_lower_to_rgb']['translation']), 0)
                lower_theta = float(global_config_dict['calibrated']['lidar_lower_to_rgb']['rotation'][-1])
                lower_rotation_matrix = np.array([[np.cos(lower_theta), -np.sin(lower_theta)], [np.sin(lower_theta), np.cos(lower_theta)]])
                lower_pc[:, :2] = np.squeeze(np.matmul(lower_rotation_matrix, np.expand_dims(lower_pc[:, :2], 2)))
                pointcloud = lower_pc
                pointcloud = np.vstack([upper_pc, lower_pc])
                pointcloud = np.hstack([pointcloud, np.ones((pointcloud.shape[0], 1))])
                torch.save(pointcloud,pc_dst)

            print('Completed copying images and pointclouds for sequence [%d/%d] in %s' % (i+1, len(sequence_paths), indir))
...
def convert_2d_gt(opt, train=True):
    ...
    if train:
        ...
        indir = opt.test
    data_columns = ['frame', 'id', 'bb_left', 'bb_top',
                    'bb_width', 'bb_height',
                    'x', 'y', 'z', 'l', 'h', 'w',
                    'theta', 'conf']
    ...
    for i, fname in enumerate(sorted(files)):
        ...
        for frame_name in sorted(orig_labels['labels'].keys()):
            ...
            for target in frame_info:
                ...
                data.append([int(frame_num), int(id_num),
                             box_attrs[0], box_attrs[1],
                             box_attrs[2], box_attrs[3],
                             int(-1), int(-1), int(-1),
                             int(-1), int(-1), int(-1),
                             int(-1), occlusion])
...
def convert_stitched_det(opt, train=True):
    ...
    # load the files in the directory
    if train:
        files = glob(os.path.join(opt.train, 'detections', 'detections_2d_stitched', '*.json'))
        seq_folder = 'sequences'
        indir = opt.train
    else:
        files = glob(os.path.join(opt.test, 'detections', 'detections_2d_stitched', '*.json'))
        seq_folder = 'test_sequences'
        indir = opt.test
...
def convert_3d_gt(opt, train=True):
    train_seqs, test_seqs = get_train_test_names()
    ...
    if train:
        ...
        files = glob(os.path.join(opt.test, 'detections', 'detections_3d', '*.json'))
        seq_folder = 'test_sequences'
        indir = opt.test
    ...
    for i, fname in enumerate(sorted(files)):
        ...
        # loop over each frame Train_sequences
        #for frame_name in sorted(orig_labels['labels'].keys()):
        #    frame_info = orig_labels['labels'][frame_name]


        # loop over each frame test_sequences
        for frame_name in sorted(orig_labels['labels'].keys()):
            frame_info = orig_labels['lablels'][frame_name]
            # loop over targets
            for target in frame_info:
                id_num = int(target['label_id'].split(':')[-1])
                box = target['box']
                frame_num = int(frame_name.split('.')[0])
                conf = 1
                rotation_y = (-box['rot_z']
                      if box['rot_z'] < np.pi else 2 * np.pi -
                      box['rot_z'])
        ...
        save_path = os.path.join(opt.outdir, seq_folder, video, 'gt')
        save_name = os.path.join(save_path, '3d_gt.txt')
        os.makedirs(save_path, exist_ok=True)
        df.to_csv(save_name, index=False, header=False)

        print('Saved 3d gt [%d/%d]' % (i+1, len(files)))

def get_prediction_type_files(opt, train=True):
    train_seqs, test_seqs = get_train_test_names()

    # load the files in the directory
    if train:
        files = glob(os.path.join(opt.train, 'labels', 'labels_3d', '*.json'))
        seq_folder = 'sequences_predict/'
        indir = opt.train
    else:
        files = glob(os.path.join(opt.test, 'labels', 'labels_3d', '*.json'))
        seq_folder = 'test_sequences'
        indir = opt.test
    ...
    for i, fname in enumerate(sorted(files)):
        ...
        save_path = os.path.join(opt.outdir)
        save_name = os.path.join(save_path, seq_folder, video+'.txt')
        os.makedirs(save_path, exist_ok=True)
        df.to_csv(save_name, index=False, header=False)
...
def copy_calib(opt, train=True):
    ...
    for sequence in sequences:
        shutil.copytree(os.path.join(opt.train, 'calib'), os.path.join(opt.outdir, seq_folder, sequence, 'calib'))
...
def main(opt):
    ...
    #get_prediction_type_files(opt, train=True)
    #copy_calib(opt)
