...
from .lap_util import linear_sum_assignment
...
class MOTAccumulator(object):
    ...
    def update(self, oids, hids, dists, frameid=None, gt_dists=None, det_dists=None):
        ...
        if no * nh > 0:
            for i in range(no):
                for j in range(nh):
                    ...
                    if gt_dists is None:
                        self._events.append(['RAW', oids[i], hids[j], dists[i,j], np.nan])
                    else:    
                        self._events.append(['RAW', oids[i], hids[j], dists[i,j], gt_dists[i].item()])
        elif no == 0:
            for i in range(nh):
                self._indices.append((frameid, next(eid)))
                if det_dists is None:
                    self._events.append(['RAW', np.nan, hids[i], np.nan, np.nan])
                else:
                    self._events.append(['RAW', np.nan, hids[i], np.nan, det_dists[i].item()])
        elif nh == 0:
            for i in range(no):
                self._indices.append((frameid, next(eid)))
                if gt_dists is None:
                    self._events.append(['RAW', oids[i], np.nan, np.nan, np.nan])
                else:
                    self._events.append(['RAW', oids[i], np.nan, np.nan, gt_dists[i].item()])
        ...
        if oids.size * hids.size > 0:    
            ...
            for i in range(oids.shape[0]):
                ...
                if np.isfinite(dists[i,j]):
                    ...
                    if gt_dists is not None:
                        self._events.append(['MATCH', oids.data[i], hids.data[j], dists[i, j], gt_dists[i].item()])
                    else:
                        self._events.append(['MATCH', oids.data[i], hids.data[j], dists[i, j], np.nan])
            ...
            for i, j in zip(rids, cids):                
                ...
                self._indices.append((frameid, next(eid)))
                if gt_dists is not None:
                    self._events.append([cat, oids.data[i], hids.data[j], dists[i, j], gt_dists[i].item()])
                else:
                    self._events.append([cat, oids.data[i], hids.data[j], dists[i, j], np.nan])
        ...
        if gt_dists is not None:
            for o, g in zip(oids[~oids.mask], gt_dists[~oids.mask]):
                self._indices.append((frameid, next(eid)))
                self._events.append(['MISS', o, np.nan, np.nan, g.item()])
        else:
            for o in oids[~oids.mask]:
                self._indices.append((frameid, next(eid)))
                self._events.append(['MISS', o, np.nan, np.nan, np.nan])
        
        # 4. All remaining hypotheses are false alarms
        if det_dists is not None:
            for h, d in zip(hids[~hids.mask], det_dists[~hids.mask]):
                self._indices.append((frameid, next(eid)))
                self._events.append(['FP', np.nan, h, np.nan, d.item()])
        else:
            for h in hids[~hids.mask]:
                self._indices.append((frameid, next(eid)))
                self._events.append(['FP', np.nan, h, np.nan, np.nan])
    ...
    def new_event_dataframe():
        ...
        df = pd.DataFrame(
            OrderedDict([
                ...
                ('dist', pd.Series(dtype=float))     # Distance of GT object (tracked object for FP) from origin    
            ]),
            index=idx
        )
        return df
    ...
    def new_event_dataframe_with_data(indices, events):
        ...
        series = [
            ...
            pd.Series(tevents[4], dtype=float, name='dist')
        ]
